...
openssl_error::openssl_error(const std::string& msg) : ssl::ssl_error(msg) {
}

openssl_error::openssl_error(int code) : ssl::ssl_error(ERR_error_string(code, nullptr)) {
}

void openssl_error::test(int code) {
  if (code != 0) {
    throw openssl_error(code);
  }
}

driver::state::state(SSL_CTX* native_context, ssl::mode mode) : _native_context(native_context) {
  _mode = mode;
...
void driver::state::handshake(std::function<void()>& on_handshake) {
  _on_handshake = std::move(on_handshake);

  handshake();
}

bool driver::state::ready() {
  return SSL_is_init_finished(_native_state) != 0;
}

void driver::state::decrypt(std::string_view data) {
...
void driver::state::encrypt(std::string_view data, std::function<void(std::exception_ptr)> cb) {
  if (!ready()) {
...
void driver::state::onReadDecrypted(std::function<void(std::string_view)>& value) {
  _on_read_decrypted = std::move(value);
}

void driver::state::onWriteEncrypted(std::function<void(std::string&&, std::function<void(std::exception_ptr)>)>& value) {
  _on_write_encrypted = std::move(value);
}

std::string_view driver::state::protocol() {
...
int driver::state::getError(int rc) {
  return SSL_get_error(_native_state, rc);
}

void driver::state::sendPending(std::function<void(std::exception_ptr)> cb) {
...
driver::context::context(ssl::mode mode) : ssl::driver::context() {
...
std::shared_ptr<ssl::driver::state> driver::context::createState() {
  return std::make_shared<state>(_native_context, _mode);
}

void driver::context::useCertificateFile(const char* path, ssl::filetype type) {
...
void driver::context::usePrivateKeyFile(const char* path, ssl::filetype type) {
...
void driver::context::useCertificateChainFile(const char* path) {
...
void driver::context::useALPNProtocols(const std::vector<std::string>& protocols) {
...
void driver::context::useALPNCallback(std::function<bool(std::string_view)>& cb) {
...
void driver::context::validateCertificateAndPrivateKey() {
  if (++_certkey_count == 2) {
...
int driver::context::mapFiletype(ssl::filetype type) {
  switch (type) {
  case ssl::PEM:
...
std::shared_ptr<ssl::driver::context> driver::getContext(ssl::mode mode) const {
