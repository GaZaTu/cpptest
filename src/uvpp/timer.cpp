#include "uvpp/timer.hpp"

namespace uv {
timer::data::data(uv_timer_t* native_timer) : _native_timer(native_timer) {
}

timer::data::~data() {
  delete _native_timer;
}

timer::timer(uv_loop_t* native_loop, uv_timer_t* native_timer)
    : handle(native_timer, new data(native_timer)), _native_timer(native_timer) {
  error::test(uv_timer_init(native_loop, native_timer));
}

timer::timer(uv_loop_t* native_loop) : timer(native_loop, new uv_timer_t()) {
}

timer::timer(uv_timer_t* native_timer) : timer(uv_default_loop(), native_timer) {
}

timer::timer() : timer(uv_default_loop(), new uv_timer_t()) {
}

timer::timer(timer&& source) noexcept
    : handle(source._native_timer, handle::getData<data>(source._native_timer)),
      _native_timer(std::exchange(source._native_timer, nullptr)) {
}

timer::operator uv_timer_t*() noexcept {
  return _native_timer;
}

timer::operator const uv_timer_t*() const noexcept {
  return _native_timer;
}

void timer::start(std::function<void()> timer_cb, uint64_t timeout, uint64_t repeat) {
  data* data_ptr = getData<data>();
  data_ptr->timer_cb = timer_cb;

  error::test(uv_timer_start(
      *this,
      [](uv_timer_t* native_timer) {
        data* data_ptr = handle::getData<data>(native_timer);
        data_ptr->timer_cb();
      },
      timeout, repeat));
}

void timer::startOnce(uint64_t timeout, std::function<void()> cb) {
  start(cb, timeout, 0);
}

#ifdef UVPP_TASK_INCLUDE
task<void> timer::startOnce(uint64_t timeout) {
  return task<void>::create([this, timeout](auto& resolve, auto& reject) {
    startOnce(timeout, resolve);
  });
}
#endif

void timer::stop() {
  error::test(uv_timer_stop(*this));
}

void timer::again() {
  error::test(uv_timer_again(*this));
}

void timer::repeat(uint64_t repeat) noexcept {
  uv_timer_set_repeat(*this, repeat);
}

uint64_t timer::repeat() const noexcept {
  return uv_timer_get_repeat(*this);
}

void timeout(uint64_t timeout, std::function<void()> cb) {
  uv::timer* timer = new uv::timer();
  timer->startOnce(timeout, [timer, cb{std::move(cb)}]() {
    delete timer;
    cb();
  });
}

#ifdef UVPP_TASK_INCLUDE
task<void> timeout(uint64_t timeout) {
  uv::timer timer;
  co_await timer.startOnce(timeout);
}
#endif
} // namespace uv
